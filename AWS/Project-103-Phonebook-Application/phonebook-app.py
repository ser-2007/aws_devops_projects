...
db_endpoint = open("/home/ec2-user/dbserver.endpoint", 'r', encoding='UTF-8') # daha once manuel rds olusturduk
# su an manuel olusturmayacagiz. Cloudformation otomatik olusturacak # it will be created automaticly
# Configure mysql database

app.config['MYSQL_DATABASE_HOST'] = db_endpoint.readline().strip() # we must be careful in this state. bu kisma dikkat etmemiz gerekmektedir
...
def insert_person(name, number):
    ...
    """
    cursor.execute(query)
    row = cursor.fetchone() # birebir esitleme
    if row is not None: 
...
def add_record():
    if request.method == 'POST':
        ...
        return render_template('add-update.html', show_result=True, result_html=result_app, not_valid=False, action_name='save', developer_name='Serkan') 
    else:
...
def update_record():
    if request.method == 'POST':
        ...
        return render_template('add-update.html', show_result=True, result_html=result_app, not_valid=False, action_name='update', developer_name='Serkan') 
    else:
...
def delete_record():
    if request.method == 'POST':
        ...
        return render_template('delete.html', show_result=True, result_html=result_app, not_valid=False, developer_name='Serkan') 
    else:
